...
class DirectiveWatcher:
    ...
    def check_github_issue_directive(self, force: bool = True) -> Optional[Directive]:
        """Check for user replies or comments on GitHub issues, including newly created issues.
        ...
        for issue_num in issues_to_check:
            try:
                ...
                issue_data = None
                if hasattr(self.correspondence_manager, "get_issue_data"):
                    issue_data = self.correspondence_manager.get_issue_data(issue_num)
                ...
                if issue_data is None and not comments:
                    if hasattr(self.correspondence_manager, "_is_issue_open") and not self.correspondence_manager._is_issue_open(issue_num):
                        logger.debug(f"Issue #{issue_num} is closed or deleted. Skipping.")
                        continue

                target_info = (
                    self.correspondence_manager.get_target_for_issue(issue_num)
                    if hasattr(self.correspondence_manager, "get_target_for_issue")
                    else None
                )
                ...
                body_marker_id = f"issue_body_{issue_num}"
                if issue_author == AUTHORIZED_DIRECTIVE_AUTHOR.lower() and body_marker_id not in self.seen_comment_ids:
                    is_adrastea_issue = (
                        is_registered_target
                        or (
                            self.correspondence_manager.is_adrastea_content(issue_body, issue_author)
                            if hasattr(self.correspondence_manager, "is_adrastea_content")
                    ...
                    if not is_adrastea_issue:
                        ...
                        has_adrastea_reply = any(
                            (
                                self.correspondence_manager.is_adrastea_content(c.get("body", ""), c.get("author", {}).get("login", ""))
                                if hasattr(self.correspondence_manager, "is_adrastea_content")
                        ...
                        if has_adrastea_reply:
                            self.seen_comment_ids.add(body_marker_id)
                            self._save_seen_comments()
                        else:
                            self.seen_comment_ids.add(body_marker_id)
                            self.last_seen_comment_id = body_marker_id
                            self._save_seen_comments()
                            if hasattr(self.correspondence_manager, "reset_issue_wait"):
                                self.correspondence_manager.reset_issue_wait(issue_num)
                            ...
                            topic = target_info.get("title") if target_info else (issue_title or f"Issue #{issue_num}")

                            logger.info(
                                f"Security Verified: Directive received from newly created Issue #{issue_num} "
                            ...
                            return Directive(
                                ...
                                goal_id=goal_id,
                                topic=topic,
                                raw_comment_id=body_marker_id,
                            )
                    else:
                        self.seen_comment_ids.add(body_marker_id)
                        self._save_seen_comments()
                elif issue_author and issue_author != AUTHORIZED_DIRECTIVE_AUTHOR.lower() and body_marker_id not in self.seen_comment_ids:
                    # External contributor opened an issue: send pleasantry
                    self.seen_comment_ids.add(body_marker_id)
                    self._save_seen_comments()
                    if hasattr(self.correspondence_manager, "post_contributor_pleasantry"):
                        logger.info(f"Issue #{issue_num} created by external user @{issue_author}. Responding with pleasantry.")
                        self.correspondence_manager.post_contributor_pleasantry(issue_num, issue_author)

                # 3. Check if issue is waiting on operator response with escalating backoff
                waiting, reason = self.correspondence_manager.is_waiting_for_user_response(issue_num, issue_data=issue_data)
                if waiting:
                ...
                if "safe to send status follow-up" in reason.lower() or "expired" in reason.lower():
                    wait_count = (
                        self.correspondence_manager.get_wait_count(issue_num)
                        if hasattr(self.correspondence_manager, "get_wait_count")
                        else 0
                    )
                    follow_up = (
                    ...
                    self.correspondence_manager.post_response_to_issue(issue_num, follow_up, force=True)
                    continue
                ...
                for idx, comment in enumerate(comments):
                    ...
                    is_adrastea_comment = (
                        self.correspondence_manager.is_adrastea_content(body, author)
                        if hasattr(self.correspondence_manager, "is_adrastea_content")
                        else (
                    ...
                    if author != AUTHORIZED_DIRECTIVE_AUTHOR.lower():
                        ...
                        self._save_seen_comments()
                        logger.warning(
                            f"Security: Ignored directive on Issue #{issue_num} from unauthorized user '@{author}'. "
                            f"Only @{AUTHORIZED_DIRECTIVE_AUTHOR} is permitted to give instructions. Sending pleasantry."
                        )
                        if hasattr(self.correspondence_manager, "post_contributor_pleasantry"):
                            self.correspondence_manager.post_contributor_pleasantry(issue_num, author)
                        continue

                    # Check if any subsequent comment is already an Adrastea reply for this directive
                    has_subsequent_adrastea_reply = any(
                        (
                            self.correspondence_manager.is_adrastea_content(nc.get("body", ""), nc.get("author", {}).get("login", ""))
                            if hasattr(self.correspondence_manager, "is_adrastea_content")
